...
const Upcoming = () => {
  return (
    <div className="flex flex-cop gap-12 max-container px-[10%] my-12">
      {UPCOMING.map((u, index) => (
        <div
          key={index}
          className="relative h-[400px] lg:h-[600px] w-full bg-orange-50 overflow-hidden cursor-pointer rounded-[1rem] group"
        >
          <Image
            src={u.image}
          ...
          <div className="w-[80%] max-w-[40rem] bg-orange-50 cursor-pointer flex-col items-start p-[1.875rem] flex absolute top-auto bottom-[7%] left-[5%] right-auto rounded-xl gap-4 ease-in-out duration-300 hover:bg-orange-90 hide">
            <div className="flex gap-2">
              ...
              <span className="py-1 px-3 border-[1px] border-black-50 rounded-xl text-xs lg:text-sm">{u.date}</span>
            </div>
            <h4 className="text-xl md:text-2xl lg:text-3xl font-extrabold !leading-normal">{u.title}</h4>
          </div>
        </div>
      ))}
    </div>
